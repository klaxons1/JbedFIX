...
Surface rendering also remains unresolved because the `libsurfaceflinger_client.so` shim has no modern display presentation path. Android 11 also blocks direct APN provider access; `JbedMidpManager` now treats APN lookup/observer failures as "no HTTP proxy" instead of crashing the remote VM process, and the legacy APN settings menu is hidden. Local JAR/JAD selections now also trigger a direct native `Jbed_ams_event_requestLocalInstall()` upcall from `AmsConnection` in addition to the original queued `INSTALL` event, because the recursive scheduler can fail to reach `NativeAms.nativeGetEvent()` after a post-foreground stack overflow.
