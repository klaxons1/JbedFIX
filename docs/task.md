...
Current experiment: `libjbedcompat.so` now uses a staged scheduler patch. During NativeAms bootstrap it changes libjbedvm's hard-coded `Jbed_run(50)` immediate to the VM's minimum accepted `Jbed_run(20)`. Once Java observes the foreground transition — or, as a fallback, once `nativeJbedRun()` throws the post-foreground `StackOverflowError` — `JbedEngine.nativeEnableLowSchedulerQuantum()` lowers the wrapper to `Jbed_run(1)` and patches the matching `Jbed_iterate` `quantum >= 20` assertion guard to `>= 1`, so install/input events can be tested without the deep zero-delay scheduler recursion. The Android host `JbedThread` remains at 16 MiB diagnostic stack headroom, and Java unblocks the AMS startup wait if nativeJbedRun still overflows after the foreground transition, so the UI is not left forever on the modal wait dialog.
